...
class Heatmap(Linkage):
    ...
    def plot(
        ...
        ax2_y = (
            ax1_y
            + ax1_h
            + height_between_ax1_axc
            + axc_h
            + height_between_axc_ax2
        )
        ax2_w = axc_w
        ...
        if self.column_method:
            ...
            ind2 = hierarchy.fcluster(
                Y, 0.7 * max(Y[:, 2]), self.cluster_criterion
            )
        ...
        if colorbar == True:
            axcb = fig.add_axes(
                [axcb_x, axcb_y, axcb_w, axcb_h], frame_on=False
            )
            ...
            cb = matplotlib.colorbar.ColorbarBase(
                axcb, cmap=cmap, norm=norm, orientation=orientation
            )
